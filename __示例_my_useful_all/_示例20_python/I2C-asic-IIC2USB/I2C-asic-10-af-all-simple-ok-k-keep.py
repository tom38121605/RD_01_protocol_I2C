...
# Display the ADC return result, comment it out to disable the extra I2C command for easy probing
    ...
    if (Sample_count==1):
        print("{0:0>4d}-{1:0>2d}-{2:0>2d}_{3:0>2d}:{4:0>2d}:{5:0>2d}.{6:0>3d},{7},{8:.2f},{9:.2f}".format(myobj.year,myobj.month,myobj.day,int(myobj.strftime("%H")),myobj.minute,myobj.second,int(myobj.microsecond/1000),int.from_bytes(FIFO[0:3],"big",signed="True"),measure,int.from_bytes(Temp_degree,"big")),file=f)
    elif (Sample_count==3):
        print("X1:",int.from_bytes(FIFO[0:3],"big",signed="True")\
            ,"Y1:",int.from_bytes(FIFO[3:6],"big",signed="True")\
            ,"Z1:",int.from_bytes(FIFO[6:9],"big",signed="True")\
        ...
        print("", int.from_bytes(FIFO[0:3], "big", signed="True"), file=f)
        # print("", int.from_bytes(FIFO[3:6], "big", signed="True"), file=f)
        # print("", int.from_bytes(FIFO[6:9], "big", signed="True"), file=f)
        ...
        z1points.append(int.from_bytes(FIFO[6:9],"big",signed="True"))
            ...
            ,"Y2:",int.from_bytes(FIFO[15:18],"big",signed="True")\
            ,"Z2:",int.from_bytes(FIFO[18:21],"big",signed="True")\
            ,"T2:",int.from_bytes(FIFO[21:24],"big",signed="True"),file=f)
    end = time.time()
    length = end - start
    if (length > 5):
        pass
